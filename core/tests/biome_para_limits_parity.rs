//! `get_biome_para_limits` parity vs cubiomes' `getBiomeParaLimits`.
...
use cubioxides::finder::biome_para::get_biome_para_limits;
...
#[test]
fn biome_para_limits_matches_cubiomes() {
    ...
    let body = &bytes[32..];
    const REC_LEN: usize = 60; // 4 (mc) + 4 (id) + 4 (has) + 12*4 (limits)

    for i in 0..count as usize {
        let r = &body[i * REC_LEN..(i + 1) * REC_LEN];
        let mc = mc_from_ord(read_i32(r, 0));
        let id = read_i32(r, 4);
        let has = read_i32(r, 8);
        let got = get_biome_para_limits(mc, id);
        if has == 0 {
            assert!(
                got.is_none(),
                "case {i} ({mc:?}, id={id}): rust returned Some, cubiomes returned NULL"
            );
        } else {
            let expected: [(i32, i32); 6] = [
                ...
                (read_i32(r, 52), read_i32(r, 56)),
            ];
            let got = got.unwrap_or_else(|| {
                panic!("case {i} ({mc:?}, id={id}): rust None, cubiomes had data")
            });
            assert_eq!(got, expected, "case {i} ({mc:?}, id={id}): limits mismatch");
        }
    }
}
