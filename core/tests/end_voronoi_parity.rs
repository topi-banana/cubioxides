//! End `gen_biomes` parity at scale=1 (voronoi) vs cubiomes'
//! `genEndScaled`. Covers both the pre-1.15 planar branch
//! (`mapVoronoi114` + Y replication) and the 1.15+ 3D branch
//! (`mapVoronoiPlane` per Y layer).
...
#[test]
fn end_voronoi_matches_cubiomes() {
    ...
    for i in 0..count as usize {
        ...
        let rx = read_i32(&bytes, offset + 12);
        let ry = read_i32(&bytes, offset + 16);
        let rz = read_i32(&bytes, offset + 20);
        let sx = read_i32(&bytes, offset + 24);
        let sy = read_i32(&bytes, offset + 28);
        let sz = read_i32(&bytes, offset + 32);
        let err = read_i32(&bytes, offset + 36);
        offset += 40;
        ...
        assert_eq!(
            got, expected,
            "case {i} (mc={mc:?}, seed={seed:#x}, area=({rx},{ry},{rz})+({sx},{sy},{sz}))",
        );
    }
}
