//! `check_for_biomes` Beta-path parity vs cubiomes' `checkForBiomes`.
...
fn fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("check_for_biomes_beta.bin")
}
...
#[test]
fn check_for_biomes_beta_matches_cubiomes() {
    ...
    for i in 0..count as usize {
        ...
        let got = match res {
            CheckForBiomesResult::Pass => 1,
            CheckForBiomesResult::Fail => 0,
            CheckForBiomesResult::ExclusionStop => 2,
            CheckForBiomesResult::Unsupported => -1,
        };
        assert_eq!(
            got, expected_result,
            "case {i} ({mc:?} seed={seed:#x} req={req:?}[..{req_len}] exc={exc:?}[..{exc_len}] any={any:?}[..{any_len}]): \
             rust {got} vs cubiomes {expected_result}",
        );
    }
}
